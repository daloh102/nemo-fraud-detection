...
def evaluate_full_dataset(num_samples=100):
    ...
    print(f"\n🚀 Starte strukturierte Auswertung von {total_samples} Datensätzen...\n")
    ...
    for i, example in enumerate(test_lines[:total_samples]):
        ...
        # Prompt, der dem Modell Raum für Argumentation gibt
        ...
        # Intelligente Extraktion aus dem begründenden Text
        # Wir suchen nach Kernbegriffen, die der Detektor nutzt
        if any(w in response_only for w in ["klassisches beispiel für einen betrug", "handelt es sich um betrug", "ist ein betrug", "betrugsversuch", "betrüger"]):
            pred = "fraud"
        elif any(w in response_only for w in ["legitimen vorgang", "legitimes szenario", "ist legitim"]):
            pred = "legitimate"
        else:
            # Fallback über einfache Worthäufigkeit
            if response_only.count("betrug") > response_only.count("legitim"):
                pred = "fraud"
            else:
                pred = "legitimate"
