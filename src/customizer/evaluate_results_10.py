...
OUTPUT_FILE = "/data/nemo-fraud-detection/results/evaluation_results_10.jsonl"

def evaluate_full_dataset(num_samples=10):
    ...
    for i, example in enumerate(test_lines[:total_samples]):
        ...
        # Datensatz für die Export-Datei vorbereiten
        file_records.append({
            "id": i + 1,
            "input": user_input,
            "ground_truth": gt,
    ...
    # Ergebnisse in separate Datei schreiben
    print(f"💾 Speichere Ergebnisse in: {OUTPUT_FILE}")
    with open(OUTPUT_FILE, "w", encoding="utf-8") as out_f:
        for record in file_records:
            out_f.write(json.dumps(record, ensure_ascii=False) + "\n")

    # Kompakte Tabellen-Ausgabe auf der Konsole
    print("\n" + "="*65)
